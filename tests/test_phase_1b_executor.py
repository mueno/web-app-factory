...
def test_quality_self_assessment_generated_after_execute(
    ...
    """Quality self-assessment JSON is written to docs/pipeline/quality-self-assessment-1b.json."""
    mod = _import_executor()
    executor = mod.Phase1bSpecExecutor()
    ctx = make_ctx(project_dir=project_dir_with_deliverables)
    ...
    assessment_path = (
        ...
        / "docs"
        / "pipeline"
        / "quality-self-assessment-1b.json"
    )
    assert assessment_path.exists(), "quality-self-assessment-1b.json was not generated"
    content = json.loads(assessment_path.read_text())
    assert content["phase_id"] == "1b"
