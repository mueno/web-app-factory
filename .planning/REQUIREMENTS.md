...
- [x] **MCPH-02**: HTTP transport entry point (`openai_mcp_server.py`) serves all 7 existing `waf_*` tools over HTTPS
- [x] **MCPH-03**: All 7 existing tools annotated with `readOnlyHint`, `destructiveHint`, `openWorldHint` per OpenAI Apps SDK spec
- [x] **MCPH-04**: `waf_` prefix CI assertion extended to cover both stdio and HTTP server tool registrations
