"""HTTP transport MCP server for Web App Factory — OpenAI Apps / ChatGPT distribution.

Entry point: ``web-app-factory-mcp-http`` (declared in pyproject.toml).
Transport: HTTP (compatible with ChatGPT and OpenAI Apps via the MCP HTTP transport).
Endpoint: POST /mcp (bound to 0.0.0.0:8000 by FastMCP).
...
Tools (identical set to mcp_server.py — stdio transport):
...
Design: Both this file and mcp_server.py are thin transport wrappers that delegate
to web_app_factory._tool_impls. Neither imports the other. A fix in _tool_impls.py
propagates to both transports automatically.
"""
...
# HTTP server — separate FastMCP instance from the stdio server.
# Same name and instructions for OpenAI tool description parity.
mcp = FastMCP(
    "web-app-factory",
    instructions=(
...
def main() -> None:
    """Entry point for the ``web-app-factory-mcp-http`` console script.

    Binds FastMCP to HTTP transport on 0.0.0.0:8000, endpoint at /mcp.
    """
